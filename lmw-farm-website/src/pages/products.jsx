...
function Products() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const [animalRes, plantRes, merchRes] = await Promise.all([
        getProductsByCategory('animal'),
        getProductsByCategory('plant'),
        getProductsByCategory('merch')
      ]);
      setAnimalProducts(animalRes.data);
      setPlantProducts(plantRes.data);
      setMerchProducts(merchRes.data);
    } catch (error) {
  ...
  const plantGrouped = groupBySubcategory(plantProducts);
  const merchGrouped = groupBySubcategory(merchProducts);
  ...
  <div className="container mx-auto px-4 text-center">
    ...
    <p className="text-xl md:text-2xl">
      Heritage breed chickens, farm-fresh produce, and quality farm merchandise
    </p>
  </div>
</section>

<div className="py-16 bg-gradient-to-r from-orange-50 to-orange-100"></div>
      <div className="container mx-auto px-4">
                  ...
                  <div key={breed} className="mb-8">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {breedChicks.map((product) => (
                        <div key={product.product_id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition">
                          ...
                          <p className="text-sm text-gray-500 mt-2">Available Spring 2025</p>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {/* Plant Products */}
        <div className="mb-16">
          <h2 className="text-4xl font-bold mb-8 text-green-700">🌿 Plant Products</h2>
          
          {Object.entries(plantGrouped).map(([subcategory, products]) => (
            <div key={subcategory} className="mb-12">
              <h3 className="text-3xl font-bold mb-6 text-gray-800 border-b-2 border-green-500 pb-2">
                {subcategory}
              </h3>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map((product) => (
                  <div key={product.product_id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition">
                    <h4 className="text-xl font-bold mb-2">{product.product_name}</h4>
                    <p className="text-gray-600 mb-4">{product.description}</p>
                    <p className="text-2xl font-bold text-green-600">${parseFloat(product.unit_price).toFixed(2)}</p>
                    <p className="text-sm text-gray-500 mt-2">Available Spring 2025</p>
                  </div>
                ))}
              </div>
        ...
        {/* Merchandise */}
        <div className="mb-16">
          <h2 className="text-4xl font-bold mb-8 text-orange-700">🛍️ Merchandise</h2>
          
          {Object.entries(merchGrouped).map(([subcategory, products]) => (
            <div key={subcategory} className="mb-12">
              <h3 className="text-3xl font-bold mb-6 text-gray-800 border-b-2 border-orange-500 pb-2">
                {subcategory}
              </h3>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map((product) => (
                  <div key={product.product_id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition">
                    <h4 className="text-xl font-bold mb-2">{product.product_name}</h4>
                    <p className="text-gray-600 mb-4">{product.description}</p>
                    <p className="text-2xl font-bold text-orange-600">${parseFloat(product.unit_price).toFixed(2)}</p>
                    <p className="text-sm text-gray-500 mt-2">Available Spring 2025</p>
                  </div>
                ))}
              </div>
